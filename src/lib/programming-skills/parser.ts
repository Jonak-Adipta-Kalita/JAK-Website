...
const buildGraphData = () => {
    ...
    addNode({ id: "tools", name: "Tools", type: "field" });
    const allTools = skills.tools.flatMap((t) =>
        "tools" in t ? t.tools : [t]
    );
    for (const tool of allTools) {
        addNode({ id: tool.id, name: tool.name, type: "tool" });
